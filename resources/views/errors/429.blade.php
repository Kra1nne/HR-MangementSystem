...
@section('content')
        ...
        <div class="d-flex justify-content-center mt-5">
            <div class="d-flex flex-column align-items-center">
                <div class="d-grid gap-2 d-md-block">
                    <a href="{{ route('login') }}" class="btn btn-success btn-lg text-center my-6 d-none" id="homeButton">
                    ...
                    <button class="btn btn-secondary btn-lg text-center my-6" id="disabledButton" disabled>
                        ...
                        Please wait <span class="badge bg-light text-dark ms-1"
                            id="buttonTimer">{{ $retry_after ?? 60 }}</span>s
                    </button>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const defaultTime = {{ $retry_after ?? 60 }};
            const storageKey = 'rate_limit_end_time';

            let endTime = localStorage.getItem(storageKey);

            // If no stored time OR already expired, set a new one
            if (!endTime || new Date().getTime() > endTime) {
                endTime = new Date().getTime() + (defaultTime * 1000);
            ...
            const countdown = setInterval(function() {
                ...
                if (timeLeft <= 0) {
                    ...
                    // Clear storage so next visit resets properly
